...
// Re-issuing a certificate whose PKCS#12 bundle was lost regenerates
// the bundle from the existing key, CSR and certificate instead of
// refusing with the duplicate error.
func TestIssueRegeneratesLostP12(t *testing.T) {
	oldBase := baseDir
	baseDir = t.TempDir()
	t.Cleanup(func() { baseDir = oldBase })
	oldOrg, oldRoot := orgName, rootCN
	...
	p12 := filepath.Join(baseDir, "servers/p12/host.example.com.p12")
	os.Remove(p12)

	logs, err := IssueServer("host.example.com", "", "rp", "")
	if err != nil {
		t.Fatalf("re-issuance should regenerate the bundle, got: %v", err)
	}
	if !strings.Contains(strings.Join(logs, "\n"), "regenerated") {
		t.Fatalf("expected a regeneration log, got: %v", logs)
	}
...
// The duplicate guard still refuses when nothing is missing.
func TestIssueDuplicateStillRefused(t *testing.T) {
	oldBase := baseDir
	baseDir = t.TempDir()
	t.Cleanup(func() { baseDir = oldBase })
	oldOrg, oldRoot := orgName, rootCN
...
// The startup sanity scan: silent on a healthy CA, repairable
// findings for a missing p12 or chain, manual for a missing key, and
// RegenerateP12/RegenerateChain clear what they repair.
func TestSanityIssues(t *testing.T) {
	oldBase := baseDir
	baseDir = t.TempDir()
	t.Cleanup(func() { baseDir = oldBase })
	oldOrg, oldRoot := orgName, rootCN
	...
	if err != nil || len(iss) != 0 {
		t.Fatalf("healthy CA must be silent, got: %v (%v)", iss, err)
	}

	os.Remove(filepath.Join(baseDir, "users/p12/u1@example.com.p12"))
	os.Remove(filepath.Join(baseDir, "users/certs/u1@example.com-chain.pem"))
	iss, err = SanityIssues()
	if err != nil || len(iss) != 2 {
		t.Fatalf("expected 2 repairable findings, got: %v (%v)", iss, err)
	}
	kinds := map[string]bool{}
	for _, s := range iss {
		kinds[s.Kind] = true
	}
	if !kinds["p12"] || !kinds["chain"] {
		t.Fatalf("expected p12 and chain findings, got: %v", iss)
	}
	if _, err := RegenerateP12("user", "u1@example.com", "up", ""); err != nil {
		t.Fatalf("RegenerateP12: %v", err)
	}
	if _, err := RegenerateChain("user", "u1@example.com"); err != nil {
		t.Fatalf("RegenerateChain: %v", err)
	}
	iss, _ = SanityIssues()
	if len(iss) != 0 {
		t.Fatalf("repairs must clear the findings, got: %v", iss)
	}
	// RegenerateP12 refuses when the bundle is already there
	if _, err := RegenerateP12("user", "u1@example.com", "up", ""); err == nil {
		t.Fatal("expected refusal when the bundle exists")
	}
}

// validDir guards the CA directory typed into the TUI: absolute,
// canonical (no trailing spaces, no .. or redundant separators), and
// built from a shell-friendly charset only.
func TestValidDir(t *testing.T) {
	good := []string{
		"/home/rc/ca-go",
		"/srv/ca",
		"/CA/Dir_1.2-x",
		"/a", // shortest reasonable
	}
	for _, s := range good {
		if !validDir(s) {
	...
	bad := []string{
		"",                 // empty
		"home/rc/ca-go",    // not absolute
		"/home/rc/ca-go ",  // trailing space
		" /home/rc/ca-go",  // leading space
		"/home/rc/ca-go/",  // trailing slash
		"/home//rc",        // redundant slash
		"/home/rc/../rc",   // ..
		"/home/rc/../rc",   // .. that would clean away
		"/home/rc/$lkjds",  // expansion
		"/home/rc;rm",      // command separator
		"/home/rc/ca #go",  // comment character
		"/home/rc/ca-go#x", // comment character, no space
		"/home/rc/ca\tgo",  // control character
		"/home/rc/ca\ngo",
		"/hömecägo",         // non-ASCII
		"/home/rc/ca:go",    // colon
		"/home/rc/ca*go",    // glob
		"/home/rc/'quoted'", // quotes
	}
	for _, s := range bad {
		if validDir(s) {
